...
语法内容要求：
- `语法` 列承担示例职责，不再单独设置 `示例` 列或示例段落，避免同一条命令重复出现
- 每条子命令至少给出 1 个最小可调用语法
- 工作区参数统一写成 `-w <workspace>`，不得使用 `~/SiYuan`、`D:\...` 等具体路径
- 如果该命令有常见的 JSON 输出或只读用法，可在 `语法` 或 `输出/副作用` 中体现
- 如果命令会修改数据，要明确提醒
- `## 备注` 中不得写 `-f json` 相关偏好，例如不要写“读取命令优先配合 `-f json`”

### 命令页统一写法
每个 `references/<command>.md` 文件用固定结构组织，正文按下列顺序编排：
...
其中 `## 子命令详述` 必须用表格形式展开，不改成段落叙述。表格列建议固定为：

| 子命令 | 用途 | 语法 | 关键参数 | 输出/副作用 | 备注 |
|---|---|---|---|---|---|

要求如下：
- 每个顶层命令下的子命令都要在表格里出现，不能只挑一部分写
- `语法` 列只保留最小可调用形式，不要展开成长段说明，也不要再在同一行另写 `示例`
- `关键参数` 列只写这条子命令真正需要的参数，通用参数可放到顶层总览里
- `输出/副作用` 列明确区分只读、写入、修改 workspace、返回 JSON、返回文本等行为
- 如果子命令会写入数据，`备注` 列必须显式标注，必要时写出 `--dry-run` 或容器块约束
- `备注` 列只写限制、风险和行为补充，不写 `-f json` 相关使用偏好
- 如果某个顶层命令只有一个子命令，仍然保留表格结构，不降级成散文
...
## 测试验证约束
写完 skill 后必须进行测试验证，避免把 CLI 用法、参数或语法写错。
