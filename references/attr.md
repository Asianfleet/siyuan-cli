...
|---|---|---|---|---|---|
| `get` | 读取单个块的属性 | `siyuan attr get --id <block-id> -w <workspace>` | `--id <block-id>`：块 ID，必填，无默认值 | 只读；默认输出按属性名排序的 `NAME / VALUE` 表格，`-f json` 时返回属性映射 | `--dry-run` 仍会真实读取；找不到块或无属性时输出 `(empty)`，不报错 |
