...
| `info` | 显示当前或默认 workspace 信息 | `siyuan workspace info [-w <workspace>]` | `-w, --workspace <workspace>`：可选，指定被检查的目标 workspace；未传时检查 CLI 解析出的默认 workspace；显式传入优先 | 只读；默认输出 `Path / Version / IsValid`，`-f json` 时返回同等字段对象 | 即使目标路径不存在或不是合法工作区，也会输出 `IsValid: false`，不会报错；`info` 只是检查路径是否像工作区，不会校验 `workspace.json` 是否已登记；`--dry-run` 与普通执行无差异 |
