...
|---|---|---|---|---|---|
| `list` | 分页列出历史记录 | `siyuan history list [--notebook <id>] [--op <op>] [--type <0-4>] [--page <n>] -w <workspace>` | `--notebook <id>`：可选，仅对文档类历史有效；默认不过滤；`--op <op>`：可选，历史操作过滤；默认不过滤，`all` 等同不过滤；`--type, -t <0-4>`：可选，历史类型；默认 `1`；`--page, -p <n>`：可选，页码；默认 `1` | 只读；表格输出按时间戳分组列出操作、标题、历史路径和分页信息；`-f json` 输出 `timestamps`、`pageCount`、`totalCount` | 没有结果时输出 `No history found.`；`--page` 小于 `1` 时按第 `1` 页查询；`--op` 不校验取值，不匹配时只会得到空结果 |
| `search` | 按关键词搜索历史记录 | `siyuan history search <query> [--notebook <id>] [--op <op>] [--type <0-4>] [--page <n>] -w <workspace>` | `<query>`：必填，搜索词，只使用第 1 个位置参数；`--notebook <id>`：可选，仅对文档名称/内容搜索有效；默认不过滤；`--op <op>`：可选，历史操作过滤；默认不过滤，`all` 等同不过滤；`--type, -t <0-4>`：可选，历史类型；默认 `1`；`--page, -p <n>`：可选，页码；默认 `1` | 只读；输出格式与 `list` 一致，但只返回命中查询词的历史记录 | 查询词含空格时必须加引号；没有位置参数会报错；没有命中时输出 `No history found.` |
